...
export default function ListForm({
  ...
  const { register, handleSubmit, errors, error, onSubmit, isSubmitting } =
    useListFormState({ list, action, onSuccess: onClose });
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-2 text-sm">
          ...
          />
          {errors.public && (
            <p className="mt-1 text-sm text-red-600">{errors.public.message}</p>
          )}
        </label>
      </div>
